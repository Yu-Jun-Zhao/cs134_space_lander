...
void ParticleSystem::update() {
	...
	if (particles.size() == 0 || !enabled)  return;
...
void ParticleSystem::toggleOnOff(bool en) {
	enabled = en;
	if (!enabled) {
		for (int i = 0; i < particles.size(); i++) {
			particles[i].reset();
...
Thruster::Thruster(ofVec3f dir) {
	this->direction = dir;
}

void Thruster::updateForce(Particle * particle) {

	particle->forces += direction * magnitude * particle->mass;
	//cout << direction << endl;
}
